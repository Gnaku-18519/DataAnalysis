...
## Limitations: transitive closure (e.g. "find all direct relatives of Fred") -> not RA, need Cond
## Query
...
### Tricks
1. "or" == for the same attribute, OR(^) on the attribute; for different attributes, Union on the subsets
2. "and" == use Intersection of relations or Join the same relation several times
3. "all" == Division
4. "some" == exist
5. "only" == all - not only
...
* Index Nested Loop Join
  ...
  * cost can vary a lot
  * E.g. Company(cname, city), Product(pname, maker, price) with B(Company) = B(Product) = 10000 blocks; each block can accomodate 5 records and the memory has 101 buffers; there is an index on cname of Company
    * put Product as **outer** relation and Company as inner relation
    * as each block accomodates 5 blocks, for each element in Product, it at most needs to be read in 5 times to compare with the elements in Company
...
* Passes -- **#Passes = ceil(log<sub>2</sub>N) + 1**
  * Pass 0: read a page, sort it, write it
  * Pass 1,2,...: use 3-buffer method
* **Total Cost = 2N * (ceil(log<sub>2</sub>N) + 1)** -- each pass we need read + write, so 2 disk I/Os
...
* Passes -- **#Passes = ceil(log<sub>B-1</sub>(ceil(N/B))) + 1**
...
<img width="525" alt="image" src="https://user-images.githubusercontent.com/84046974/208485615-5ee82a44-d846-4e55-a05f-547d3f913354.png">
